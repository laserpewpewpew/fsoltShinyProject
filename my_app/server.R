...
shinyServer(function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    if(input$compare=="Within One Country"){
      ...
      data <- data.frame(swiid[swiid$country==input$country,])
      
      if(isTRUE()=T)
      p <- arrangeGrob(
        ggplot(total, aes(x=year, y=gini_net)) + 
          geom_line() +
          ...
          labs(x = "Year", y = "SWIID Inequality Measure"),
    ...
    } else if(input$compare=="Across Multiple Countries"){
    ...
    } else if(input$compare=="Not Selected"){
      return()
    }
